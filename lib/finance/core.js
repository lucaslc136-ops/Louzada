...
export function computeCategoryBreakdown(transactions, accountsMap, monthISO) {
  const totals = {};
  let sum = 0;
  for (const t of transactions) {
  ...
  const rows = Object.entries(totals).map(([categoryId, total]) => ({
    categoryId, name: categoryById(categoryId)?.name || categoryId, total,
...
export function computeBudgetGroups(transactions, accountsMap, monthISO, receitasMes, settings) {
