...
export function computeAlerts({ accounts, transactions, debts, settings, todayISO, diasAntecedencia = 5 }) {
  ...
  for (const acc of accounts) {
    ...
    if (dias <= diasAntecedencia) {
      alerts.push({
        id: `fatura-${acc.id}`, tipo: "fatura", severidade: dias <= 1 ? "alta" : "media",
  ...
  if (settings) {
    ...
    for (const g of groups) {
      if (g.usoPct >= 90) {
        alerts.push({
          id: `orcamento-${g.group}`, tipo: "orcamento", severidade: g.usoPct >= 100 ? "alta" : "media",
  ...
  for (const d of debts) {
    ...
    if (dias >= 0 && dias <= diasAntecedencia) {
      alerts.push({
        id: `divida-${d.id}`, tipo: "divida", severidade: dias <= 1 ? "alta" : "media",
  ...
  const aguardandoRevisao = transactions.filter((t) => t.needs_review).length;
  if (aguardandoRevisao > 0) {
    alerts.push({
      id: "revisao-pendente", tipo: "revisao", severidade: "media",
      titulo: `${aguardandoRevisao} lançamento(s) importado(s) automaticamente aguardando revisão`,
